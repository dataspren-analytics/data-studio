...
const ExcelFileViewerInner = memo(function ExcelFileViewerInner({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await runtimeActions.writeFile(file, dirPath);
        ...
        >
          <div
            style={{
              ...
              height: ROW_HEIGHT + rowVirtualizer.getTotalSize(),
              position: "relative",
            }}
          >
            {/* Corner cell (empty) */}
            ...
            {selection && hasMultipleCells(selection) && !isSelecting && !showSqlInput && (
              <div
                ...
                style={{
                  top: ROW_HEIGHT + (getSelectionBounds(selection).maxRow + 1) * ROW_HEIGHT + 4,
                  left: ROW_HEADER_WIDTH + (getSelectionBounds(selection).maxCol + 1) * COL_WIDTH - 70,
                }}
              >
                <button
            ...
            {selection && showSqlInput && (
              <div
                ...
                style={{
                  top: ROW_HEIGHT + (getSelectionBounds(selection).maxRow + 1) * ROW_HEIGHT + 4,
                  left: ROW_HEADER_WIDTH + getSelectionBounds(selection).minCol * COL_WIDTH,
                }}
              >
